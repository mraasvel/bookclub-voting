...
export class FormService {
	...
	constructor(
		...
		private formAnswerRepository: Repository<FormAnswer>,
		@InjectRepository(LinearScale)
		private linearScaleRepository: Repository<LinearScale>,
		@InjectRepository(LinearScaleAnswer)
		private linearScaleAnswerRepository: Repository<LinearScaleAnswer>,
	) {}

	async createForm(owner: User, formData: FormDTO) {
		const form = this.formFromDTO(owner, formData);
		this.logger.log(
			`User: [${owner.id}] ${owner.username} creates new form: ${formData.name}`,
		);
	...
	async submitAnswer(user: User, answerData: FormAnswerDTO, formId?: number) {
		const question = await this.assertFormIsOpenByQuestionId(
			answerData.questionId,
		);
		if (formId && question.form.id !== formId) {
			throw new BadRequestException(
				`invalid answer for given formId: expected: ${question.form.id}, received: ${formId}`,
			);
		}
		this.assertFormTypeProperty(question.formQuestionType, answerData);
		const answer = this.answerFromDTO(user.id, question, answerData);
	...
	async getFormAnswers(formId: number, userId?: number) {
		...
		for (const question of form.formQuestions) {
	...
	async submitForm(
		user: User,
		formId: number,
		formSubmitData: FormSubmitDTO,
	) {
		for (const answer of formSubmitData.answers) {
	...
	async addQuestion(formId: number, questionData: FormQuestionDTO) {
		const form = await this.assertFormIsOpen(formId);
		const question = this.questionFromDTO(questionData);
	...
	async updateQuestion(
		questionId: number,
		partialQuestion: FormQuestionPartialDTO,
	) {
		const question = await this.assertFormIsOpenByQuestionId(questionId);
		if (question.formQuestionType !== partialQuestion.type) {
			throw new NotImplementedException(
				'cannot change the type of a question',
			);
		}
		switch (partialQuestion.type) {
			case FormQuestionType.LinearScale:
				question.linearScale.updateFromPartial(
					partialQuestion.linearScale,
				);
				break;
		}
	...
	private formFromDTO(owner: User, formData: FormDTO) {
		return this.formRepository.create({
			name: formData.name,
			owner,
			formQuestions: formData.questions.map((question) =>
				this.questionFromDTO(question),
			),
		});
	}
	...
	private answerFromDTO(
		userId: number,
		question: FormQuestion,
		answerData: FormAnswerDTO,
	) {
		return this.formAnswerRepository.create({
			userId,
			formQuestionId: question.id,
			formAnswerType: question.formQuestionType,
			linearScaleAnswer: this.linearScaleAnswerFromDTO(
				question,
				answerData.linearScaleAnswer,
			),
		});
	}

	private linearScaleAnswerFromDTO(
		question: FormQuestion,
		...
		if (!linearScaleAnswer) {
			...
			linearScaleAnswer.score < question.linearScale.rangeStart ||
			linearScaleAnswer.score > question.linearScale.rangeEnd
		) {
			throw new BadRequestException(
				`invalid score value for linear scale question: ${linearScaleAnswer.score}`,
			);
		}
